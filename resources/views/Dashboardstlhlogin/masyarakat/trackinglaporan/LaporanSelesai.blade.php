...
<section class="min-h-screen py-4">
    ...
    <div class="max-w-4xl mx-auto px-4 mb-4 flex items-center space-x-4">
        <form action="{{ route('masyarakat.laporan.like', $laporan->id) }}" method="POST" class="inline">
            @csrf
            <button type="submit" class="flex items-center space-x-1 text-gray-600 hover:text-blue-500 {{ isset($liked) && $liked ? 'text-blue-600' : '' }}">
      ...
      <div class="max-w-4xl mx-auto rounded-3xl shadow-md p-6 mb-8 border border-gray-400">
        ...
        <form action="{{ route('masyarakat.laporan.komentar', $laporan->id) }}" method="POST" class="flex items-center space-x-3 border-t border-black pt-4">
            ...
            <button type="submit" class="text-gray-600 p-2">
                <img src="{{ asset('img/icon/kirim.svg') }}" class="w-10 h-5">
            </button>
        </form>
        @endif
    </div>
    @endisset
            ...
            <div class="lg:col-span-5">
                <div class="bg-white rounded-xl shadow-xl p-8 h-full">
                                    ...
                                    <div class="absolute left-0 flex items-center justify-center transform -translate-x-1/2 -translate-y-3">
                                        ...
                                        <div class="pl-8">
                                            <div class="bg-green-400 p-4 rounded-xl border border-gray-100">
                                                ...
                                                <p class="text-sm text-white mt-2 ml-11">Waktu di setujui admin: {{ $updated_at->format('Y-m-d H:i:s') }}</p>
                                            </div>
                                        </div>
                            </div>
                        </div>
                    ...
                    <!-- Rating Section -->
    <div class="max-w-4xl mx-auto  rounded-3xl  shadow-md p-6 border border-gray-400 mb-4 mt-6">
        <h3 class="text-lg font-bold text-gray-800 mb-3">Berikan Nilai Kepuasanmu terhadap penanganan layanan laporan ini.</h3>
        <form action="{{ route('masyarakat.laporan.kirimUlasan', $laporan->id) }}" method="POST">
            @csrf
            <div class="flex items-center justify-center space-x-4 mb-4">
                @for($i=1; $i<=5; $i++)
                    <button type="button" class="star-btn" data-value="{{ $i }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-gray-300 hover:text-yellow-500" viewBox="0 0 20 20" fill="currentColor">
            ...
            <textarea name="ulasan" required class="w-full p-3 border border-gray-300 rounded-lg resize-none" rows="3" placeholder="berikan tanggapanmu..."></textarea>
            <button type="submit" class="w-full mb-4 mt-4 bg-yellow-400 text-black font-bold py-3 px-4 rounded-lg hover:bg-yellow-500 transition-colors flex items-center justify-center gap-2">
                Kirim Ulasan
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        starButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                // highlight bintang sesuai value
                document.querySelectorAll('.star-btn svg').forEach((svg, idx) => {
                    svg.classList.toggle('text-yellow-500', idx < value);
